    ...
    if platform.system() == "Windows":
        try:
            ...
            with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Shell Folders") as key:
                # Get the value of the "Personal" key, which points to the user's documents directory
...
def get_cursor_path():
    """Get Cursor path based on OS"""
    if platform.system() == "Windows":
        # In Windows, the path is usually in LocalAppData
        local_app_data = os.environ.get('LOCALAPPDATA')
        if local_app_data:
            return os.path.join(local_app_data, 'Programs', 'Cursor', 'resources', 'app')
        else:
            # Fallback if LOCALAPPDATA is not set
            return os.path.expanduser("~\\AppData\\Local\\Programs\\Cursor\\resources\\app")
    elif platform.system() == "Darwin":
        # In macOS, it's in the Applications folder
        return "/Applications/Cursor.app/Contents/Resources/app"
    else:
        # For Linux, use the existing function
        return get_linux_cursor_path()
...
def get_default_brave_driver_path():
    ...
    # Brave browser is based on Chromium, so it uses the same chromedriver
    return get_default_chrome_driver_path()
...
def get_default_browser_path(browser_type='chrome'):
    ...
    if sys.platform == "win32":
        if browser_type == 'chrome':
            ...
            # Try multiple possible Opera paths
            opera_paths = [
            ...
            for path in opera_paths:
                if os.path.exists(path):
                    return path
            return opera_paths[0]  # Return the first path even if it doesn't exist
        elif browser_type == 'operagx':
            # Try multiple possible Opera GX paths
            operagx_paths = [
            ...
            for path in operagx_paths:
                if os.path.exists(path):
                    return path
            return operagx_paths[0]  # Return the first path even if it doesn't exist
        elif browser_type == 'brave':
            # Default installation paths for Brave browser
            paths = [
    ...
    # If the specified browser type is not found, return the path for Chrome
