...
import struct
import json
from utils import get_cursor_path
...
def run(translator=None):
    ...
    try:
        ...
        if is_authorized:
            ...
            try:
                user_json_path = os.path.join(get_cursor_path(), "user.json")
                if os.path.exists(user_json_path):
                    ...
                    if plan.lower() == "pro":
                        print(f"{Fore.GREEN}{EMOJI['INFO']} {translator.get('check_user.pro_plan_unlimited', default='Pro plan: Unlimited prompts.')}{Style.RESET_ALL}")
                    else:
                        accepted_prompts = 0
                        limit_prompts = 0
                        for item in usage:
                            if item.get("feature") == "fast":
                                accepted_prompts = item.get("accepted", 0)
                                limit_prompts = item.get("limit", 0)
                                break
                        prompts_left = limit_prompts - accepted_prompts
                        print(f"{Fore.GREEN}{EMOJI['INFO']} {translator.get('check_user.prompts_left', default='Fast prompts left')}: {prompts_left}/{limit_prompts}{Style.RESET_ALL}")
            except Exception as e:
                print(f"{Fore.YELLOW}{EMOJI['WARNING']} {translator.get('check_user.usage_error', default='Could not retrieve prompt usage')}: {e}{Style.RESET_ALL}")
        else:
